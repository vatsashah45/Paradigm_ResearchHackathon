"""
v2: Fully parallel, zero loops — 12.42ms (#5→#6)

Key insight: Reshape all q_blocks into the batch dimension to process
everything in a single batched GEMM. Eliminated the Python for-loop.
This was a 3x speedup from v1.
"""
...
def block_sparse_attn_fwd(q, k, v, row_ptr, col_idx, seq_lens):
    ...
    # Gather all KV block indices at once
    ...
    # Single batched GEMM for all Q blocks
