"""
v5: bf16 gather + logsumexp — intermediate, tested locally only

Experiment: Keep K/V in bf16 during gather to halve memory bandwidth,
convert to f32 only for the matmul. Also tried torch.logsumexp for
numerically stable softmax in one fused call.

This was an intermediate experiment that informed later versions.
...
def block_sparse_attn_fwd(q, k, v, row_ptr, col_idx, seq_lens):
    ...
    # Keep K/V in bf16 for gathering (save bandwidth)
    ...
    # Gather in bf16 (half the memory traffic)
