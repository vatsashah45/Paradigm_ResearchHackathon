"""
v3: torch.compile attempt — REJECTED (compile fails in sandbox)

Added torch.compile decorator to the core computation function.
Worked locally but the evaluation sandbox rejects it because compilation
times out or fails in the restricted environment.

This version was never scored — it was rejected at submission.
"""
...
def _compiled_attn(q_flat, gathered_k, gathered_v, mask, q_valid, N):
...
def block_sparse_attn_fwd(q, k, v, row_ptr, col_idx, seq_lens):
    """Same as v2 but wraps core compute in torch.compile. REJECTED by sandbox."""
    ...
    out, lse = _compiled_attn(q_flat, gathered_k, gathered_v, mask, q_valid, N)
