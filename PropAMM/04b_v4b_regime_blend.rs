...
pub const NAME: &str = "VatsaEdge v4b";
...
// Quote-time (compute_swap) — P0 base + P2 calm adaptation
...
// P2-like calm regime params
const P2_ARB_K: u128 = 15_000 * BPS;
const P2_COUNTER_K: u128 = 3_000 * BPS;
const P2_SIZE_K: u128 = 2_700 * BPS;
const P2_SHOCK_K: u128 = 1_200 * BPS;
const P2_TOX_K: u128 = 10 * BPS;
const SIGMA_CALM_THRESH: u128 = 5_000_000;
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    // Regime blend: calm → P2-like, volatile → P0-like
    let calm_w = if init_flag == 0 || sigma >= SIGMA_CALM_THRESH {
        0u128
    } else {
        // Linear ramp: full P2 below 1M, full P0 above SIGMA_CALM_THRESH
        let low = 1_000_000u128;
        if sigma <= low { SCALE } else { wdiv(SIGMA_CALM_THRESH - sigma, SIGMA_CALM_THRESH - low) }
    };
    let vol_w = SCALE - calm_w;

    // Blend parameters
    let arb_k = wmul(P2_ARB_K, calm_w) + wmul(P0_ARB_K, vol_w);
    let counter_k = wmul(P2_COUNTER_K, calm_w) + wmul(P0_COUNTER_K, vol_w);
    let size_k = wmul(P2_SIZE_K, calm_w) + wmul(P0_SIZE_K, vol_w);
    let shock_k = wmul(P2_SHOCK_K, calm_w) + wmul(P0_SHOCK_K, vol_w);
    let tox_k = wmul(P2_TOX_K, calm_w) + wmul(P0_TOX_K, vol_w);
