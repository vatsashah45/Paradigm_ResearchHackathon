...
pub const NAME: &str = "VatsaEdge v2";
...
const S_PHAT_FAST:  usize = 32;
const S_FEE_FLOOR:  usize = 40;
const S_TOX:        usize = 48;
const S_SHOCK:      usize = 56;
const S_LAST_SPOT:  usize = 64;
const S_SUM_W:      usize = 72;
const S_SUM_WP:     usize = 80;
const S_CNT:        usize = 88;
const S_STEP_VOL:   usize = 96;
const S_CNT_EMA:    usize = 104;

// =====================================================================
// Fee parameters — matched to reference P0 profile + improvements
// =====================================================================
const FEE_MIN: u128 = 1 * BPS;
const FEE_MAX: u128 = 1000 * BPS;
const DEFAULT_FEE: u128 = 25 * BPS;
const INITIAL_PRICE: u128 = 100 * SCALE;
const INIT_QUOTE_FEE: u128 = 55 * BPS; // higher initial fee for uninitialized state

// Quote-time parameters (from reference P0)
const ARB_K: u128 = 5_200 * BPS;
const COUNTER_K: u128 = 1_000 * BPS;
const SHOCK_READ_K: u128 = 733 * BPS;
const TOX_READ_K: u128 = 74 * BPS;
const PRICE_DEV_K: u128 = 400 * BPS;
const REBATE_RISK_MULT: u128 = 1_500_000_000;

// v2 improvement: directional size penalties
const SIZE_K_ARB: u128 = 3_800 * BPS;     // higher for arb-dir (push arbs to normalizer)
const SIZE_K_COUNTER: u128 = 2_200 * BPS;  // lower for counter-dir (attract retail)
const SIZE_K_DEFAULT: u128 = 2_900 * BPS;  // fallback
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    // Reference price: blend slow/fast p_hat weighted by shock
    ...
    let fee = if init_flag == 0 {
        ...
        clamp_fee(INIT_QUOTE_FEE + wmul(PRICE_DEV_K, deviation))
    } else {
        ...
        if is_arb_dir {
            ...
            // Counter-direction rebate with risk dampening
    ...
    // Directional size penalty (v2 innovation)
    let size_k = if init_flag == 0 {
        SIZE_K_DEFAULT
    } else if is_arb_dir {
        SIZE_K_ARB
    } else {
...
pub fn after_swap(data: &[u8], storage: &mut [u8]) {
    ...
    let gamma = if fee_floor < SCALE { SCALE - fee_floor } else { 0 };
    ...
    // Sigma update (first trade per step only)
