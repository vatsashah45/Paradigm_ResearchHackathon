...
pub const NAME: &str = "VatsaEdge v6";
...
const CALM_SIGMA_THRESH: u128 = 3_500_000;
const CALM_SHOCK_THRESH: u128 = 3_000_000;
const CALM_FEE_DISCOUNT: u128 = 3 * BPS;
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    let fee = if init_flag == 0 {
        ...
        // Calm-market flat discount: tighter spreads when vol and shock are low
        let sigma_val = rd64(storage, S_SIGMA) as u128;
        if sigma_val < CALM_SIGMA_THRESH && shock_hat < CALM_SHOCK_THRESH {
            f = f.saturating_sub(CALM_FEE_DISCOUNT);
        }
        clamp_fee(f)
    };
