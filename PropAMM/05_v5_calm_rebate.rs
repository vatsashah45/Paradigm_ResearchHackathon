...
pub const NAME: &str = "VatsaEdge v5";
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    let fee = if init_flag == 0 {
        ...
        if is_arb_dir {
            ...
            // Calm-market boost: extra rebate when both sigma and shock are very low
            let sigma_val = rd64(storage, S_SIGMA) as u128;
            if sigma_val < 2_000_000 && shock_hat < 2_000_000 {
                let calm_rebate = wmul(500 * BPS, dev2);
                f = f.saturating_sub(calm_rebate);
            }
        }
        clamp_fee(f)
    };
