...
pub fn after_swap(data: &[u8], storage: &mut [u8]) {
    ...
    if new_step {
        ...
        // Flow-adaptive floor decay: faster decay when cnt_ema is low (normalizer winning)
        let base_fd = if shock_hat > 15_000_000 { FLOOR_DECAY_HI } else { FLOOR_DECAY };
        let fd = if cnt_ema < 300_000_000 && init_flag != 0 && step > 100 {
            // Very low flow: decay faster to lower fees
            base_fd.saturating_sub(100_000_000).max(700_000_000)
        } else if cnt_ema > 1_200_000_000 {
            // High flow: we're competitive, decay slower to keep higher fees
            base_fd.saturating_add(30_000_000).min(960_000_000)
        } else {
            base_fd
        };
