...
// =====================================================================
// Storage layout (232 bytes used of 1024)
// =====================================================================
const S_FEE:        usize = 0;
...
const S_FLOOR_1:    usize = 176; // per-profile fee floor for P1
const S_FLOOR_2:    usize = 184; // per-profile fee floor for P2
...
// Profile 1 (max defense) — structurally higher fees
const P1_F_BASE_FEE:   u128 = 22 * BPS;
const P1_F_VOL_MULT:   u128 = 700_000_000;
const P1_F_TOX_QUAD:   u128 = 18_000 * BPS;
const P1_F_TOX_CUBE:   u128 = 60_000 * BPS;
const P1_F_SHOCK_QUAD: u128 = 150_000 * BPS;
const P1_F_SHOCK_CUBE: u128 = 110_000 * BPS;
// Profile 2 (loss minimizer) — structurally lower fees
const P2_F_BASE_FEE:   u128 = 10 * BPS;
const P2_F_VOL_MULT:   u128 = 500_000_000;
const P2_F_TOX_QUAD:   u128 = 12_000 * BPS;
const P2_F_TOX_CUBE:   u128 = 45_000 * BPS;
const P2_F_SHOCK_QUAD: u128 = 100_000 * BPS;
const P2_F_SHOCK_CUBE: u128 = 80_000 * BPS;
...
// =====================================================================
// Compute per-profile fee target
// =====================================================================
#[inline(never)]
fn compute_profile_target(
    profile: u8, sigma: u128, tox: u128, tox2: u128,
    shock_hat: u128, sh2: u128, sh3: u128,
    size_fee: u128, tox_shock_spike: bool, sv_bump: u128,
) -> u128 {
    let (base, vm, tq, tc, sq, sc) = match profile {
        0 => (P0_BASE_FEE, P0_VOL_MULT, P0_TOX_QUAD, P0_TOX_CUBE, P0_SHOCK_QUAD, P0_SHOCK_CUBE),
        1 => (P1_F_BASE_FEE, P1_F_VOL_MULT, P1_F_TOX_QUAD, P1_F_TOX_CUBE, P1_F_SHOCK_QUAD, P1_F_SHOCK_CUBE),
        _ => (P2_F_BASE_FEE, P2_F_VOL_MULT, P2_F_TOX_QUAD, P2_F_TOX_CUBE, P2_F_SHOCK_QUAD, P2_F_SHOCK_CUBE),
    };
...
pub fn after_swap(data: &[u8], storage: &mut [u8]) {
    ...
    let mut fee_floor_1 = rd64(storage, S_FLOOR_1) as u128;
    let mut fee_floor_2 = rd64(storage, S_FLOOR_2) as u128;
    ...
    if init_flag == 0 {
        ...
        fee_floor_1 = DEFAULT_FEE;
        fee_floor_2 = DEFAULT_FEE;
        ...
        wr64(storage, S_FEE_1, DEFAULT_FEE as u64);
        wr64(storage, S_FEE_2, DEFAULT_FEE as u64);
    } else {
        let mut p = 0usize;
    ...
    if init_flag != 0 {
        ...
        if pre_ok && rx_pre > 0 && ry_pre > 0 {
            ...
            let stored_fees_pre = [
                rd64(storage, S_FEE) as u128,
                rd64(storage, S_FEE_1) as u128,
                rd64(storage, S_FEE_2) as u128,
            ];
            ...
            while p < NUM_PROFILES as u8 {
                scores[p as usize] = score_one_profile(
                    side, input, rx_pre, ry_pre, init_flag,
                    spot_pre, p_ref_pre, stored_fees_pre[p as usize], tox_pre, shock_pre,
    ...
    if new_step {
        ...
        let decay_pow = pow_wad(fd, elapsed);
        fee_floor = wmul(fee_floor, decay_pow);
        fee_floor_1 = wmul(fee_floor_1, decay_pow);
        fee_floor_2 = wmul(fee_floor_2, decay_pow);
        cnt = 0;
        step_vol = 0;
    }
    ...
    // Common intermediates for fee target computation
    let reserve_in = if side == 0 { ry } else { rx };
    ...
    // Per-profile fee floor updates
    let target_0 = compute_profile_target(0, sigma, tox, tox2, shock_hat, sh2, sh3, size_fee, tox_shock_spike, sv_bump);
    if target_0 > fee_floor { fee_floor = target_0; }

    let target_1 = compute_profile_target(1, sigma, tox, tox2, shock_hat, sh2, sh3, size_fee, tox_shock_spike, sv_bump);
    if target_1 > fee_floor_1 { fee_floor_1 = target_1; }

    let target_2 = compute_profile_target(2, sigma, tox, tox2, shock_hat, sh2, sh3, size_fee, tox_shock_spike, sv_bump);
    if target_2 > fee_floor_2 { fee_floor_2 = target_2; }

    // Write all state
    wr64(storage, S_FEE, clamp_fee(fee_floor) as u64);
    wr64(storage, S_FEE_1, clamp_fee(fee_floor_1) as u64);
    wr64(storage, S_FEE_2, clamp_fee(fee_floor_2) as u64);
    wr64(storage, S_FLOOR, fee_floor as u64);
    wr64(storage, S_FLOOR_1, fee_floor_1 as u64);
    wr64(storage, S_FLOOR_2, fee_floor_2 as u64);
