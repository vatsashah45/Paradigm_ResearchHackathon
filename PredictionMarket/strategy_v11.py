...
class Strategy(BaseStrategy):
    """
    v11: Keep v8's base_hs=2 safety but increase sizes and add more levels.
    """
    ...
    def on_step(self, state: StepState):
        ...
        base_size = 4.5 * inv_scale * time_scale
        bid_factor = 1.0 if bid_hs >= 3 else 0.6
        ask_factor = 1.0 if ask_hs >= 3 else 0.6
        ...
        # Third level - far out, safe from arb
        if steps_remaining > 50:
            l3_bid = max(1, bid_tick - 6)
            l3_ask = min(99, ask_tick + 6)
            l3_size = max(0.3, round(base_size * 0.25, 2))
