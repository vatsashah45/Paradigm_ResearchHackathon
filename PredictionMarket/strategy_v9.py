...
class Strategy(BaseStrategy):
    """
    v9: Two-tier regime quoting.
    
    Visual spread mapping:
      spread_ticks=1: visual 1-2
      spread_ticks=2: visual 3-4
      spread_ticks=3: visual 5-6
      spread_ticks=4: visual 7-8
    
    Regime A (visual >= 7, i.e. spread_ticks=4): Aggressive quoting (proven profitable)
    Regime B (visual 5-6, i.e. spread_ticks=3): Conservative quoting (wider spreads, smaller size)
    Regime C (visual <= 4, i.e. spread_ticks 1-2): Don't quote
    """
    ...
    def __init__(self) -> None:
        ...
        self._regime: str = "unknown"  # "wide4", "mid3", "tight"
    ...
    def on_step(self, state: StepState):
        ...
        if self._regime == "tight" or self._regime == "unknown":
            if steps_remaining < 6 and abs_inv > 0.5:
                self._flatten(actions, net_inv, abs_inv, fair_int, state)
            return actions

        if self._regime == "wide4":
            return self._quote_wide4(actions, state, step, steps_remaining, net_inv, abs_inv, fair, fair_int)
        elif self._regime == "mid3":
            return self._quote_mid3(actions, state, step, steps_remaining, net_inv, abs_inv, fair, fair_int)

        return actions

    def _quote_wide4(self, actions, state, step, steps_remaining, net_inv, abs_inv, fair, fair_int):
        """Aggressive quoting for spread_ticks=4 regime."""
        max_inv = 40.0
    ...
    def _quote_mid3(self, actions, state, step, steps_remaining, net_inv, abs_inv, fair, fair_int):
        """Conservative quoting for spread_ticks=3 regime. Wider spread, smaller size."""
        max_inv = 20.0
        inv_scale = max(0.1, 1.0 - abs_inv / (max_inv * 1.3))
        time_scale = 1.0
        if steps_remaining < 80:
        ...
        buy_cooldown = max(0, 5 - (step - self._last_buy_fill_step))
        sell_cooldown = max(0, 5 - (step - self._last_sell_fill_step))

        vol_adj = min(3, int(self._vol_ema * 0.5))
        inv_adj = min(3, int(abs_inv / 8.0))
        time_adj = 0
        if steps_remaining < 200:
            time_adj = 1
        if steps_remaining < 50:
        ...
        # Wider base half-spread for spread_ticks=3
        bid_hs = 3 + vol_adj + inv_adj + time_adj + buy_cooldown
        ask_hs = 3 + vol_adj + inv_adj + time_adj + sell_cooldown
        ...
        bid_hs = max(3, min(15, bid_hs))
        ask_hs = max(3, min(15, ask_hs))

        # Smaller sizes
        base_size = 1.5 * inv_scale * time_scale

        bid_size = max(0.3, round(base_size * 0.7, 2))
        ask_size = max(0.3, round(base_size * 0.7, 2))
        ...
        if steps_remaining > 15 and net_inv < max_inv:
            cost = bid_tick / 100.0 * bid_size
            if state.free_cash > cost + 0.3:
                actions.append(PlaceOrder(side=Side.BUY, price_ticks=bid_tick, quantity=bid_size))
