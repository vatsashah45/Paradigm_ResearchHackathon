...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self._cooldown_bid > 0: self._cooldown_bid -= 1
        if self._cooldown_ask > 0: self._cooldown_ask -= 1
        ...
        time_scale = max(0.1, min(1.0, steps_remaining / 100.0)) if steps_remaining < 100 else 1.0
        size = max(0.5, round(8.0 * inv_scale * time_scale, 2))
        self._last_posted_size = size

        bid_tick = max(1, min(98, int(round(fair - 2))))
        ask_tick = max(2, min(99, int(round(fair + 2))))
        bid_tick = min(bid_tick, comp_bid + 1)
        ask_tick = max(ask_tick, comp_ask - 1)
        if bid_tick >= ask_tick: bid_tick = max(1, ask_tick - 1)
        ...
        if steps_remaining < 6 and abs_inv > 0.5:
            ...
            flat = round(max(0.5, min(abs_inv * 0.5, 5)), 2)
            if net_inv > 0:
