...
class Strategy(BaseStrategy):
    """
    Adaptive market-making strategy with:
    1. Competitor-spread-aware quoting (aggressive when wide, conservative when tight)
    2. Asymmetric spreads based on directional fill pressure
    3. Post-fill cooldown to avoid momentum continuation losses
    4. Inventory skew for mean-reversion
    5. Size-at-spread coupling (smaller when tighter)
    """
    ...
    def on_step(self, state: StepState):
        ...
        # When competitor is wide, we have room to profit
        # When tight, there's no margin between our quote and fair value
        if ecs >= 3.5:
            base_hs = 2
            size_mult = 1.3
        elif ecs >= 2.5:
        ...
        # Post-fill cooldown
        buy_cooldown = max(0, 3 - (step - self._last_buy_fill_step))
        sell_cooldown = max(0, 3 - (step - self._last_sell_fill_step))

        # Vol and inventory adjustments
        ...
        # Quote sizes - smaller at tight spreads
        base_size = 2.0 * size_mult * inv_scale * time_scale
        bid_factor = 1.0 if bid_hs >= 4 else 0.6
        ask_factor = 1.0 if ask_hs >= 4 else 0.6
