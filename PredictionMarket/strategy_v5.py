...
class Strategy(BaseStrategy):
    """
    v5: Refined strategy with better edge in medium-spread regimes.
    
    Key changes:
    - In tight spread (1-2): don't quote
    - In medium spread (3): quote wider, with smart side selection
    - In wide spread (4): aggressive retail capture
    - Longer post-fill cooldown
    - Better fair value tracking
    """
    ...
    def on_step(self, state: StepState):
        ...
        # Don't quote in very tight regimes
        if ecs < 2.8:
        ...
        # Competitor-spread dependent parameters
        if ecs >= 3.8:
            base_hs = 2
            size_mult = 1.5
        elif ecs >= 3.2:
        ...
        # Second level
        if steps_remaining > 20 and size_mult >= 0.8:
