...
class Strategy(BaseStrategy):
    """
    v4: Skip quoting entirely in tight-spread regimes.
    Key changes:
    - If competitor spread <= 2, barely quote at all (essentially skip)
    - If competitor spread >= 3, quote aggressively to capture retail
    - This should eliminate losses in tight regimes while keeping
      profits in wide regimes
    """
    ...
    def on_step(self, state: StepState):
        ...
        if comp_bid is not None and comp_ask is not None:
            ...
            self._estimated_comp_spread = 0.93 * self._estimated_comp_spread + 0.07 * comp_spread
        ...
        # In tight spread regimes, don't quote at all - just take 0 edge
        if ecs < 2.3:
            # Only do settlement flatten if we somehow have inventory
