...
class Strategy(BaseStrategy):
    """v20: Tighter L2 (bid-2/ask+2), L3 (bid-4/ask+4). base=10, hs=2."""
    ...
    def on_step(self, state: StepState):
        ...
        if steps_remaining > 40:
            l3_bid = max(1, bid_tick - 4)
            l3_ask = min(99, ask_tick + 4)
            l3_size = max(0.3, round(base_size * 0.4, 2))
            ...
            if net_inv > -max_inv:
                avail = max(0.0, state.yes_inventory - size - (l2_size if steps_remaining > 15 else 0))
