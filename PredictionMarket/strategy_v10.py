...
class Strategy(BaseStrategy):
    """
    v10: skip non-spread_ticks=4 regimes. Optimize spread_ticks=4 quoting.
    Changes from v8: tighter base_hs=1 in spread_ticks=4, larger sizes, 3 levels.
    """
    ...
    def on_step(self, state: StepState):
        ...
        bid_hs = 1 + vol_adj + inv_adj + time_adj + buy_cooldown
        ask_hs = 1 + vol_adj + inv_adj + time_adj + sell_cooldown
        ...
        bid_hs = max(1, min(12, bid_hs))
        ask_hs = max(1, min(12, ask_hs))

        base_size = 4.0 * inv_scale * time_scale
        ...
        if steps_remaining > 20:
            l2_bid = max(1, bid_tick - 2)
            l2_ask = min(99, ask_tick + 2)
        ...
        # Third level
        if steps_remaining > 50:
            l3_bid = max(1, bid_tick - 5)
            l3_ask = min(99, ask_tick + 5)
            ...
            if net_inv > -max_inv:
                avail = max(0.0, state.yes_inventory - ask_size - l2_size if steps_remaining > 20 else state.yes_inventory - ask_size)
