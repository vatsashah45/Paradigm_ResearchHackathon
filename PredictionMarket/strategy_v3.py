...
class Strategy(BaseStrategy):
    """
    v3: Refined competitor-spread-aware market maker.
    Changes from v2:
    - Even more conservative in tight-spread regimes
    - More aggressive sizing in wide-spread regimes  
    - Faster drift adaptation
    - Better fair value tracking using competitor mid changes
    """
    ...
    def __init__(self) -> None:
        ...
        self._estimated_comp_spread: float = 2.5
        self._prev_comp_mid: float | None = None
    ...
    def on_step(self, state: StepState):
        ...
        if comp_bid is not None and comp_ask is not None:
            ...
            self._estimated_comp_spread = 0.95 * self._estimated_comp_spread + 0.05 * comp_spread
        ...
        # Track competitor mid changes as directional signal
        comp_mid_delta = 0.0
        if self._prev_comp_mid is not None:
            comp_mid_delta = comp_mid - self._prev_comp_mid
        self._prev_comp_mid = comp_mid
        ...
        # Fair value: follow competitor mid + drift + comp_mid changes
        # The comp_mid_delta gives us immediate directional info
        self._fair_value = comp_mid + self._drift_ema * 1.5 + comp_mid_delta * 0.5
        ...
        # ─── COMPETITOR SPREAD ADAPTATION ────────────────────────────
        ecs = self._estimated_comp_spread

        if ecs >= 3.5:
            # Wide competitor: quote aggressively, capture lots of retail
            base_hs = 2
            size_mult = 1.5
        elif ecs >= 2.5:
            base_hs = 2
            ...
            # Tight competitor: be cautious
            base_hs = 4
            size_mult = 0.35
        else:
            # Very tight: barely quote, protect capital
            base_hs = 5
            size_mult = 0.2

        # Directional pressure
        buy_pressure = self._buy_fill_ema
        sell_pressure = self._sell_fill_ema
        ...
        if buy_pressure > sell_pressure + 0.04:
            bid_hs += 2
        if sell_pressure > buy_pressure + 0.04:
            ask_hs += 2

        bid_hs = max(2, min(14, bid_hs))
        ask_hs = max(2, min(14, ask_hs))
        ...
        # ─── SECOND LEVEL ────────────────────────────────────────────
        if steps_remaining > 20 and size_mult >= 0.5:
