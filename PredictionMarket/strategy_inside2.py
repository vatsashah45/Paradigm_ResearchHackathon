...
class Strategy(BaseStrategy):
    """
    Inside-competitor v2: Regime-adaptive sizing.
    Quote at (comp_bid+1, comp_ask-1) with size dependent on competitor spread.
    """
    ...
    def on_step(self, state: StepState):
        ...
        # Need at least 3 ticks gap to fit inside
        if comp_spread < 3:
            return actions
        ...
        if comp_spread >= 7:        # spread_ticks=4
            base_size = 20.0
            cooldown_len = 2
        elif comp_spread >= 5:      # spread_ticks=3
            base_size = 8.0
            cooldown_len = 3
        else:                       # spread_ticks=2 (comp_spread 3-4)
            base_size = 3.0
            cooldown_len = 4
