...
class Strategy(BaseStrategy):
    """
    Regime-selective single-level market maker.

    Only quotes in spread_ticks=4 regimes (identified by visual competitor
    spread >= 7 ticks), where we can place orders inside the competitor's
    spread and profitably capture retail flow. In tighter regimes the
    arb losses exceed retail gains, so we stay flat.
    """
    ...
    def on_step(self, state: StepState):
        ...
        # Only quote in wide-spread regimes
